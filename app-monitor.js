...
const CLOUD='https://snag-media-api.nirav2000-github.workers.dev/app-monitor',DEVICE_KEY='app-monitor.v1.device';
...
const deviceId=()=>{let x=localStorage.getItem(DEVICE_KEY);if(!x){x=makeId('d-').replace(/[^A-Za-z0-9._-]/g,'');localStorage.setItem(DEVICE_KEY,x)}return x};
...
function device(){
 ...
 return{kind,browser,os,touchPoints:touch,screen:(screen?.width||0)+'x'+(screen?.height||0),viewport:innerWidth+'x'+innerHeight,pixelRatio:devicePixelRatio||1,language:navigator.language||'',timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||''};
}
function norm(u){if(!u)return null;const uid=clean(u.uid||u.userId||u.id,180),username=clean(u.username||u.displayName||u.name,120),provider=clean(u.provider||u.providerId||(u.providerData&&u.providerData[0]?.providerId),80),isAnonymous=typeof u.isAnonymous==='boolean'?u.isAnonymous:undefined;if(!uid&&!username&&!provider&&isAnonymous===undefined)return null;return{uid,username,provider,isAnonymous}}
function autoIdentity(){const a=[window.APP_MONITOR_IDENTITY,window.currentUser,window.auth?.currentUser,window.firebaseAuth?.currentUser];try{if(window.firebase?.auth)a.push(window.firebase.auth().currentUser)}catch{}for(const x of a){const n=norm(x);if(n)return n}return null}
...
addEventListener('popstate',()=>setTimeout(notePage,0));document.addEventListener('visibilitychange',()=>{tick();if(document.visibilityState==='hidden')send('hidden',true)});addEventListener('pagehide',()=>{tick();send('pagehide',true)});addEventListener('online',()=>send('online'));setInterval(()=>send('heartbeat'),5*60*1000);
