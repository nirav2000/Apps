...
function create(config={}){
  ...
  let sessionToken='',sessionExpiresAt='';
  ...
  async function startBootstrap(){const r=await fetcher('/bootstrap/request',{method:'POST'});if(!r.ok)throw new Error(await r.text());return r.json()}
  async function bootstrapStatus(requestId){const r=await fetcher('/bootstrap/status?requestId='+encodeURIComponent(requestId));if(!r.ok)throw new Error(await r.text());return r.json()}
  async function bootstrapRegister(requestId,label='First passkey'){const r=await fetcher('/bootstrap/register/options',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({requestId})});if(!r.ok)throw new Error(await r.text());const d=await r.json(),cred=await navigator.credentials.create({publicKey:creationOptions(d.options)});const v=await fetcher('/bootstrap/register/verify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({requestId,challengeId:d.challengeId,label,response:regResponse(cred)})});if(!v.ok)throw new Error(await v.text());const out=await v.json();saveSession(out.token,out.expiresAt);resetFailures();return out}
